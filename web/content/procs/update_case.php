...
if (!function_exists("GetSQLValueString")) {
 ...
 if(isset($_POST) && $_POST['case_matter'] != "")
 {
		...
		elseif($countAddedAttendeesData < $countNewAddedAttendeesFromPost)
		{
			...
			while($rowimportDocketId = mysqli_fetch_array($ResultData))
			{
					...
					foreach($TotalJurisdictionsList as $jurisData)
					{
						...
						$trigger_event_id = $capi->CreateCalendarEvent(''.$calendar_id.'', $summary, $all_day, $date_array, $user_timezone, $_SESSION['access_token'],$attendee,$description,$reminder_minutes,$eventColor,$location,$status);
		...
		elseif($countAddedAttendeesData > $countNewAddedAttendeesFromPost)
		{
			...
			while($rowimportDocketId = mysqli_fetch_array($ResultData))
			{
					...
					foreach($TotalJurisdictionsList as $jurisData)
					{
						...
						$trigger_event_id = $capi->CreateCalendarEvent(''.$calendar_id.'', $summary, $all_day, $date_array, $user_timezone, $_SESSION['access_token'],$attendee,$description,$reminder_minutes,$location,$status);
   ...
   header("Location: /update-case?case_id=".$_POST['case_id']);
    die();
 }
