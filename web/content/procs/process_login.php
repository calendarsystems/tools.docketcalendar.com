...
$stmt = Db::getInstance()->prepare("SELECT user_id FROM attorneys WHERE username = ? AND password = ?");
$stmt->bind_param("ss", $colname_rs,$colname2_rs);
$stmt->execute();
...
if ($stmt->num_rows == 1) {
	...
	if ($array['string'] != "") {
		...
		//echo "found in DM<br>";
				...
				$query_userinfo = sprintf("SELECT * FROM users WHERE username = %s", GetSQLValueString($_POST['usernamelog'], "text"));
				...
				{
					$response = "NoGmail";
					$_SESSION['author_id'] =$row_rs['email'];
					$_SESSION['CheckAccess'] ="NoGmail";
