			...
			$countNewAddedAttendeesFromPost = count($newAddedAttendeesFromPost);
				 ...
				 if ($selectJurisdiction != 0) 
				{
					...
					foreach($TotalJurisdictionsList as $jurisData)
					{
						...
						$trigger_event_id = $capi->CreateCalendarEvent(''.$calendar_id.'', $summary, $all_day, $date_array, $user_timezone, $_SESSION['access_token'],$attendees,"Trigger Custom Comments : ".mysqli_real_escape_string($docketDataSubscribe,$_POST['triggerCustomText'])."<br/>".$description,$reminder_minutes,$eventColor,$location,$status);
