...
if ($numresults > 0) {
	if ($numresults > 1) {
        $result .= '<div> <span> All Events </sapn>&nbsp;&nbsp;<span style="'.$DonotStyle.'">(Do Not Recalculate Events)</span>&nbsp;&nbsp;<span style="color:#696969">(Parent Event)</span>&nbsp;&nbsp;<span><a href="javascript:void(0);" onclick="javascript:parent.normal_view();">show normal view</a></span></div>';
	} else {
  ...
  <table class="triggers">';
    $eventResultsArray = array();
		...
		foreach ($response as $Event) {
			...
			$eventDoNotRecalculate = $Event['CalendarRuleEvent']['DoNotRecaltulateFlag'];

            $tree_array[$eventSystemID]['SystemID'] =  $eventSystemID;
            $tree_array[$eventSystemID]['ParentSystemID'] =  $eventParentSystemID;
			 $tree_array[$eventSystemID]['DoNotRecalculate'] =  $eventDoNotRecalculate;
        ...
        function HTMLMenu($menu,$do_not_recalculate_events_val, $parentid = 0)
        {
            ...
            foreach ($menu as $item) if ($item["ParentSystemID"] == $parentid)
            {
                ...
                $result1 .= "<li class='container' style=".$styleColor." ><input type='checkbox' ".$checkVal." onclick='checkResult(".$item["SystemID"].",this)' class='evenetClass' id='".$item["SystemID"].'_'.$item["ParentSystemID"]."' data='".$item["ParentSystemID"]."' name='events[]'  value='".$item["SystemID"]."' />" . $item["ShortName"]  . " " .
                HTMLMenu($menu,$do_not_recalculate_events_val,$item["SystemID"]) . "</li>\n";
            }
            return $result1 ?  "\n<ul class='tree'>\n$result1</ul>\n" : null;
        }
        ...
        $res = HTMLMenu($tree_array,$do_not_recalculate_events_val);
