	...
	{		
		...
		if($_POST['updateCaseValue'] == "AttendeeData")
		{
			...
			if (!empty($_POST['attendee'])) {
				...
				}elseif($countAddedAttendeesData > $countNewAddedAttendeesFromPost)
				{
					...
					{		
						...
						$querygetupdateeventsdescInfo = "SELECT * FROM updateeventsdesc WHERE importdocketid = '".$importDocketId."' and caseid = ".$_POST['case_id']."";
						...
						WHERE dc.case_id = ".$_POST['case_id']." AND i.import_docket_id =".$importDocketId." ORDER BY c.import_event_id desc";
				...
				$trigger_event_id = $capi->CreateCalendarEvent(''.$row_importDocketId['calendar_id'].'', $summary, $all_day, $date_array, $user_timezone, $_SESSION['access_token'],$_POST['attendee'],$description,$reminder_minutes,$eventColor,$location,$status);
			...
			} else 
			{
				...
				if($events_array != '')
				{
					...
					$queryForMultipleUpdateEvents="SELECT eventdesc,eventid FROM updateeventsdesc WHERE importdocketid=".$importDocketId." AND caseid = ".$_POST['case_id']." AND eventid = '".$sysID."'";
					...
					$insertSQLCRON = "INSERT INTO CRON_sendbulkevent(calendar,summary, allday, start_time,end_time,event_date, usertimezone, session, attendee,description,remindermin,location,status,emailsend)  VALUES ('".$row_importDocketId['calendar_id']."','".$summary."','".$all_day."','".$start_time."','".$end_time."','".$event_date."','".$user_timezone."','".$_SESSION['access_token']."','".implode(",",$_POST['attendee'])."','".$description."','".$reminder_minutes."','".$location."','".$status."',1)";
						$result = mysqli_query($docketDataSubscribe,$insertSQLCRON) or trigger_error("Query Failed! SQL: $insertSQLCRON - Error: ".mysqli_error($docketDataSubscribe), E_USER_ERROR);
		...
		elseif($countAddedAttendeesData < $countNewAddedAttendeesFromPost)
				...
				$courtRule = mysqli_real_escape_string($docketDataSubscribe,$courtRuleTextValue);					
							...
							$insertSQLCRON = "INSERT INTO CRON_sendbulkevent(calendar,summary, allday, start_time,end_time,event_date, usertimezone, session, attendee,description,remindermin,location,status,emailsend)  VALUES ('".$row_importDocketId['calendar_id']."','".$summary."','".$all_day."','".$start_time."','".$end_time."','".$event_date."','".$user_timezone."','".$_SESSION['access_token']."','".implode(",",$_POST['attendee'])."','".$description."','".$reminder_minutes."','".$location."','".$status."',1)";
							$result = mysqli_query($docketDataSubscribe,$insertSQLCRON ) or trigger_error("Query Failed! SQL: $insertSQLCRON - Error: ".mysqli_error(), E_USER_ERROR);
