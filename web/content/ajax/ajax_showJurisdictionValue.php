...
if (isset($_SESSION['userid']))
{
	...
	if($numJuris == 1)
	{
		$result_html['jurisdictionResultVal'] = $theTotalJurisdictions['Description'];
	}else{
	
		foreach($theTotalJurisdictions as $juris)
		{
		 
			if($juris['SystemID'] == $jurisdicationVal)
			{
			  $result_html['jurisdictionResultVal'] = $juris['Description'];
...
if ($triggerVal != 0)
{
        $result_html['tiggersResultVal']=  $triggerVal;
        $command="/triggers?";
        ...
        $theTriggers=$array['ArrayOfTriggerItem']['TriggerItem'];
         ...
         if ($numTriggers>0) {
          $x=0;
          do { 
            if ($theTriggers[$x]['SystemID']==$triggerVal)
            {
              $result_html['tiggersResultVal'] = $theTriggers[$x]['Description'];  
            }
             $x=$x+1;
           }while ($x < $numTriggers);
          }
