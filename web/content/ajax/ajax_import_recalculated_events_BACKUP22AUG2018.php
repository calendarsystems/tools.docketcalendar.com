...
if (!function_exists("GetSQLValueString"))
{
    function GetSQLValueString($docketData,$theValue, $theType, $theDefinedValue = "", $theNotDefinedValue = "")
    {
      ...
      return $theValue;
		...
		$getCaseId = "SELECT case_id,calendar_id from import_docket_calculator where import_docket_id ='".$importDocketId ."'";
					$caseInfo = mysqli_query($docketDataSubscribe,$getCaseId);
					$rowcaseId = mysqli_fetch_array($caseInfo);
					$caseId =$rowcaseId['case_id'];
					$calendar_id = $rowcaseId['calendar_id'];
...
/*Start Delete all Data */
					...
					$caseId =$rowcaseId['case_id'];
					$calendar_id = $rowcaseId['calendar_id'];
					$query_importEvents = "SELECT i.access_token,e.authenticator,c.event_date,i.import_docket_id,c.short_name,dc.case_matter,c.case_event_id,e.event_docket,e.has_child,i.triggerItem FROM docket_cases dc
				...
				while($rowCaseEventId = mysqli_fetch_assoc($ImportEvents))
				{
					...
					{
						$importEventIdVal =  $row_searchInfoval['event_id'];
...
if($serialize_events != '')
{
    $updateSQL = "UPDATE import_docket_calculator SET  events = '".$serialize_events."' WHERE import_docket_id = ".$importDocketId." ";
    $result = mysqli_query($docketDataSubscribe,$updateSQL) or trigger_error("Query Failed! SQL: $insertSQL - Error: ".mysqli_error(), E_USER_ERROR);
					...
					$updateImportDocketSQL = "UPDATE import_docket_calculator SET trigger_date = '".$txtTriggerDate."',trigger_time='".$txtTime."',meridiem='".$meridiem."' WHERE import_docket_id  = ".$importDocketId." ";
					 ...
					 $resultUpdateImportdocket = mysqli_query($docketDataSubscribe,$updateImportDocketSQL) or trigger_error("Query Failed! SQL: $updateSQL - Error: ".mysqli_error(), E_USER_ERROR);
...
if ($selectJurisdiction != 0) {
   // echo "juris good";
    if ($selectTriggerItem != 0) {
		...
		$LinkToEditCaseEvent = "<br>http://googledocket.com/update-calendar-event?id=";
        ...
        $event_id = $capi->CreateCalendarEvent(''.$calendar_id.'', $summary, $all_day, $date_array, $user_timezone, $_SESSION['access_token'], $attendees, $description,$reminder_minutes,$location,$status);
            ...
            $resultCases = mysqli_query($docketDataSubscribe,$insertSQLCases) or trigger_error("Query Failed! SQL: $insertSQLCases - Error: ".mysqli_error(), E_USER_ERROR);
                        ...
                        if($events_array != '')
                        {
                            ...
                            foreach ($tree_array as $key => $Event) {
							  ...
							  /*if($DND == "true")
							  {
								  $index = array_search($sysID, $events_array);
								  unset($events_array[$index]);
							  }*/
                                ...
                                if(count($attendees) > 0)
                                {
                                    ...
                                    if (!empty($attendees)) {
                                        ...
                                        foreach ($attendees as $attendeeVal) {
                                            $insertAttendeesSQLCases = "INSERT INTO docket_cases_attendees (case_id, attendee) VALUES ('".$row_searchInfo['case_id']."','".$attendeeVal."')";
