...
if($_POST['case_matter'] != "")
{
	...
	function UpdateCaseDetails($caseId,$capi,$attendee,$eventColor)
	{
		...
		$queryUpdateEventsTableForSendStatus = "UPDATE events set update_case_status = 2,session='".$_SESSION['access_token']."' WHERE eventIdvalue =".$rowEventsIdForImportDocketIdData['import_event_id']."";
		$userUpdateEventsTableForSendStatus = mysqli_query($docketDataSubscribe,$queryUpdateEventsTableForSendStatus);
		
		$queryUpdateCronJOBIsRun = "UPDATE `cronjobrun` set isRun = 1 where id = 2";
		...
		{
			while($rowAllImportDocketIdForCase=mysqli_fetch_assoc($dataAllImportDocketIdForCase))
			{
