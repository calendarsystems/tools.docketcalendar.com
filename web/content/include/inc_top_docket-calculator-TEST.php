...
echo date('Y-m-d H:i:s'). ": before  requires <br />";
...
echo date('Y-m-d H:i:s'). ": after  requires <br />";
...
echo date('Y-m-d H:i:s'). ": before mysql_select_db <br />";
mysqli_select_db($docketData,$database_docketData);
echo date('Y-m-d H:i:s'). ": after mysql_select_db <br />";

echo date('Y-m-d H:i:s'). ": start get uerinfo from DB <br />";

$query_userInfo = sprintf("SELECT * FROM attorneys WHERE user_id = %s", GetSQLValueString($colname_userInfo, "int"));
$userInfo = mysqli_query($docketData,$query_userInfo) or die(mysqli_error($docketData));
$row_userInfo = mysqli_fetch_assoc($userInfo);
$totalRows_userInfo = mysqli_num_rows($userInfo);
echo date('Y-m-d H:i:s'). ": end get uerinfo from DB <br />";
...
if (!isset($_GET['cmbJurisdictions']) && !isset($_GET['cmbTriggers']) && !isset($_POST['cmbJurisdictions']) && !isset($_GET['e'])) {
	header('Location: ?cmbJurisdictions='.$masterCourt);
}
...
//// get parentLoginToken of user
	...
	//$parameters="password=".$row_userInfo['password']."&soapREST=REST";
...
//print_r ($row_usercartInfo);


	if (isset($_POST['cmbJurisdictions']) || isset($_GET['cmbJurisdictions'])) {
		$selectJurisdiction=$_POST['cmbJurisdictions'].$_GET['cmbJurisdictions'];
	}
	if ($selectJurisdiction != 0) {
		...
		if (isset($_POST['cmbTriggers']) || isset($_GET['cmbTriggers']) ) {
			$selectTriggerItem=$_POST['cmbTriggers'].$_GET['cmbTriggers'];
		}
	}
...
//		        
		if ($isServed=="Y") {	
			...
			if (isset($_GET[cmbJurisdictions])) {
				$parameters="loginToken=$loginToken&jurisdictionSystemID=$_GET[cmbJurisdictions]";
			} else {
	...
	//	echo "trigger good";
		...
		//	echo "service good";
			if (($isTimeRequired=="Y" && $_POST['txtTime']!="") || $isTimeRequired!="Y") {
				if ($_POST['btnCalculate'] != '' || $_POST['btnExport'] != '') {
			//	echo "time good";
				//
				//// do the calculation!
				//
...
// EXPORT TO EXCEL
			if ($_POST['btnExport'] != '' && $_POST['radioExportType'] == 'excel') {				
...
// EXPORT TO ICAL
			if ($_POST['btnExport'] != '' && $_POST['radioExportType'] == 'ical') {				
...
// EXPORT TO VCAL
			if ($_POST['btnExport'] != '' && $_POST['radioExportType'] == 'vcal') {				
...
echo date('Y-m-d H:i:s'). ": the end of inc_top<br />";	
		//exit();
}
