jQuery(document).ready(function($) {
	...
	var genesis_slider_toggles = [
		['#genesis_slider_settings\\[post_type\\]', '#genesis-slider-taxonomy', 'page']
	];

	$.each( genesis_slider_toggles, function( k, v ) {
		$( v[0] ).live( 'change', function() {
			genesis_slider_toggle_settings( v[0], v[1], v[2] );
		});
		genesis_slider_toggle_settings( v[0], v[1], v[2] ); // Check when page loads too.
	});

	function genesis_slider_toggle_settings( selector, show_selector, check_value ) {
		if (
	...
	function genesis_slider_checklist_toggle() {
	...
	genesis_slider_checklist_toggle();
