			...
			$totalRows_userInfo = mysqli_num_rows($userDataForUserDetailsNeededToSendCalendarEvents);
				if($totalRows_userInfo > 0)
				{
						...
						while($rowUserData = mysqli_fetch_assoc($userDataForUserDetailsNeededToSendCalendarEvents))
					{
						...
						$querygetupdateeventsdescInfo = "SELECT eventreminderval,eventpopupreminderval FROM updateeventsdesc WHERE importdocketid = '".$rowUserData['importdocketid']."' and caseid = ".$rowUserData['caseid']."";
						...
						*/
						...
						$event_id = $capi->CreateCalendarEvent(''.$rowUserData['calendar'].'', $summary, $rowUserData['allday'], $date_array, $rowUserData['usertimezone'], $rowUserData['session'], $arrAttendees ,$rowUserData['description'],$rowUserData['remindermin'],$rowUserData['popupreminder'],$rowUserData['eventColor'],$rowUserData['location'],$rowUserData['status'],$addReminderFlag,$addReminderVal,$addPopUpReminderVal);
