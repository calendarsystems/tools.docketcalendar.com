			...
			if (($isTimeRequired == "Y" && $_POST['txtTime'] != "") || $isTimeRequired != "Y") {
				...
				if ($_POST['cmbMatter'] != "") {
					//$matterString = "(" . $_POST['cmbMatter'] . ") ";
					  $query_docketCasename = "SELECT case_matter from docket_cases where case_id = ".$_POST['cmbMatter']." ";
					  $ResultEvents = mysqli_query($docketDataSubscribe,$query_docketCasename);
					  $row_caseInfo = mysqli_fetch_assoc($ResultEvents);
					  $matterString =  $row_caseInfo['case_matter'];
				} else {
					$matterString = "";
				}
...
if ($numresults > 0) {
	if ($numresults > 1) {
        $result .= '<div><input type="checkbox" onchange="checkAll(this)"/>Select All</div>&nbsp;&nbsp;&nbsp;<div id="asc_link" style="margin-top: -45px;padding-left: 132px;"><a href="javascript:void(0);" onclick="javascript:parent.sort_date(1);">Sort by Date <img src="https://tools.docketcalendar.com/wp-content/themes/calendarrules/images/arrow-up.gif"></a></div>
        <div id="desc_link" style="margin-top: -45px;padding-left: 132px;display:none;"><a href="javascript:void(0);" onclick="javascript:parent.sort_date(2);">Sort by Date <img src="https://tools.docketcalendar.com/wp-content/themes/calendarrules/images/arrow-down.gif"></a></div>&nbsp;&nbsp;&nbsp;<div style="margin-top: -45px;padding-left: 270px;"><a href="javascript:void(0);" onclick="javascript:parent.parent.tree_view();">Show tree view</a>
        </div>';
        $result .= '<div style="margin-top: -19px;padding-left: 410px;"><h4>RESULTS (' . $numresults . ')</h4></div>';
         $result .= '<div style="margin-top: -32px;padding-left: 520px;"><a href="javascript:void(0);" onclick="javascript:PrintResult();">Print</a> <a style="padding-left:15px;" href="javascript:void(0);" onclick="javascript:Exlexport();">Excel</a>
		 <a style="padding-left:15px;" href="javascript:void(0);" onclick="javascript:CSVexport();">CSV</a>
		<a style="padding-left:15px;" href="javascript:void(0);" onclick="javascript:Icalexport();">ICal</a>
		<a style="padding-left:15px;" href="javascript:void(0);" onclick="javascript:Outlookexport();">OutLook</a>
        </div>';
	} else {
        $result .= '<div><h4>RESULTS (' . $numresults . ')</h4></div>';
		$result .= '&nbsp;&nbsp;<div style="margin-top: -52px;padding-left: 180px;margin-bottom: 20px;"><a href="javascript:void(0);" onclick="javascript:parent.PrintResult();">Print</a><a style="padding-left:15px;" href="javascript:void(0);" onclick="javascript:Exlexport();">Excel</a><a style="padding-left:15px;" href="javascript:void(0);" onclick="javascript:CSVexport();">CSV</a><a style="padding-left:15px;" href="javascript:void(0);" onclick="javascript:Icalexport();">ICal</a><a style="padding-left:15px;" href="javascript:void(0);" onclick="javascript:Outlookexport();">OutLook</a></div>';
	}
  ...
  <table class="triggers">';
	...
	if (isset($single['Action'])) {
        ...
        if($explode_event_date[1] != "00:00:00")
        {
		   ...
		   if($event_specific_time)
		   {
				$eventSpecificTime = $event_specific_time;
		   }
		...
		$IsOnDNCListOne = $response['CalendarRuleEvent']['IsOnDNCList'];
        /*if(@$_POST['import_docket_id'] != '')
        {
           $selected = " checked='checked' ";
        }*/
		
			if($IsOnDNCListOne == "true")
			{
				$selected  = "";
			}
			else
		...
		$result .= '<input type="checkbox" class="evenetClass" name="events[]" '.$selected.' value="'.$sysID.'" style="float: left;margin: 5px -11px;"/><ul class="triggersnav"  style="padding: 0 0 0 15px !important;">
        <li>
		...
		*/
		...
		{
			$result .= '<ul class="triggersnav"><li><a href="javascript:void(0);"><span class="court_rule"><b>Event Type:</b></span> ' . $response['CalendarRuleEvent']['EventType']['Description']. '</li></a></ul>';
		}
		...
		foreach ($response as $Event) {
			...
			$IsOnDNCList = $Event['CalendarRuleEvent']['IsOnDNCList'];
			if($IsOnDNCList == 'true')
			{
				$selected  = "";
			}
			...
			$result .= '<ul class="triggersnav" style="padding: 0 0 0 15px !important;">
                       <li><input type="checkbox" '.$selected.'  onclick="checkResult('.$sysID.',this)" id='.$sysID.'_'.$parentID.' data='.$parentID.' class="evenetClass" name="events[]" '.$selected_child.' value="'.$sysID.'" style="float: left;margin: 5px -17px;"/><a  href="javascript:void(0);">' . $montharray[$mo] . ' ' . substr($justdate, 8, 2) . ', ' . substr($justdate, 0, 4)  .' '.$event_specific_time.  ' - ';
								...
								/*
								Commented for changes done to show date rule separately
				...
				{
					$result .= '<ul class="triggersnav"><li><a href="javascript:void(0);"><span class="court_rule"><b>Event Type:</b></span> ' . $Event['CalendarRuleEvent']['EventType']['Description']. '</li></a></ul>';
				}
				
				    if (isset($Event['CalendarRuleEvent']['DateRules']['Rule']['RuleText'])) {
				// IF THERE IS ONE RULE
				$result .= '<ul class="triggersnav"><li><a href="javascript:void(0);"><span class="court_rule"><b>Date Rule:</b></span> ' . $Event['CalendarRuleEvent']['DateRules']['Rule']['RuleText'] . '</li></a></ul>';
				...
				// IF THERE IS ONE RULE
				$result .= '<ul class="triggersnav"><li><a href="javascript:void(0);"><span class="court_rule"><b>Court Rule:</b></span> ' . $Event['CalendarRuleEvent']['CourtRules']['Rule']['RuleText'] . '</li></a>';
				...
				// IF THERE ARE MANY RULES
					...
					$result .= '<ul class="triggersnav">
                                <li>
                                <a href="javascript:void(0);"><span class="court_rule"><b>Court Rule:</b></span> ' . $rule['RuleText'] . '</li></a></ul>';
					?>  <?php
                    }
			   }
			$result .= '</ul></ul>';
 ...
 function checkResult(val, parentElem)
 {
   ...
   if($eventSpecificTime)
   {
	
	  if($eventSpecificTime == "08:00:00")
			 {
					$result .='<script>
