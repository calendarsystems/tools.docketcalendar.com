...
if (!function_exists("GetSQLValueString")) {
	...
	
	{
	  ...
	  return $theValue;
		...
		$url = $uri . "compute/preview?loginToken=" . $loginToken."&jurisdictionSystemID=".$selectJurisdiction."&triggerItemSystemID=".$selectTriggerItem;
		...
		// CODE TO GET EXCLUDE EVENTS
		$urlexclude="http://www.crcrules.com/CalendarRulesService.svc/rest/events/donotcreatelist?loginToken=".$loginToken."&jurisdictionSystemID=".$selectJurisdiction."&triggerItemSystemID=".$selectTriggerItem;
		$sessionex = curl_init($urlexclude);
		curl_setopt($sessionex, CURLOPT_HEADER, 0);
		curl_setopt($sessionex, CURLOPT_RETURNTRANSFER, true);
		curl_setopt($sessionex, CURLOPT_HTTPHEADER, array('Content-Type: application/xml'));
		$responseex = curl_exec($sessionex);
		curl_close($sessionex);
		$startex = strpos($responseex, "<ArrayOfCalendarRulesEvent");
		$endex = strlen($responseex);
		$lengthex = $endex - $startex;
		$xmlblockex = substr($responseex, $startex, $lengthex);
		$responseex = xml2array($xmlblockex);
		$responseex = $responseex['ArrayOfCalendarRulesEvent']['CalendarRulesEvent'];
		if(!empty($response))
		{
			$getEventArray = array();
			if(empty($response[0]))
			...
			if(!empty($responseex))
			{
				if(empty($responseex[0]))
				{
					$getEventArray[] = $responseex['SystemID'];
				}
				else{
					foreach($responseex as $resultData)
						{
							$getEventArray[] = $resultData['SystemID'];
						}
				}
			}else
			{
				$urlexclude="http://www.crcrules.com/CalendarRulesService.svc/rest/events/donotcreatelist?loginToken=".$loginToken."";
				$sessionex = curl_init($urlexclude);
				curl_setopt($sessionex, CURLOPT_HEADER, 0);
				curl_setopt($sessionex, CURLOPT_RETURNTRANSFER, true);
				curl_setopt($sessionex, CURLOPT_HTTPHEADER, array('Content-Type: application/xml',"&jurisdictionSystemID=".$selectJurisdiction."&triggerItemSystemID=".$selectTriggerItem));
				$responsenew = curl_exec($sessionex);
				curl_close($sessionex);
				$startex = strpos($responsenew, "<ArrayOfCalendarRulesEvent");
				$endex = strlen($responsenew);
				$lengthex = $endex - $startex;
				$xmlblock = substr($responsenew, $startex, $lengthex);
				$responsenew = xml2array($xmlblock);
				$responsenew = $responsenew['ArrayOfCalendarRulesEvent']['CalendarRulesEvent'];
				$getEventArray = array();
				if(empty($responsenew[0]))
				{
					$getEventArray[] = $responsenew['SystemID'];
				}
				else{
					foreach($responsenew as $resultData)
						{
							$getEventArray[] = $resultData['SystemID'];
						}
				}
			...
			if($numresults == 1)
			{
					...
					$result .= '<tr><td>';
					$result .='<input type="checkbox" '.$selected.'  onclick="checkResult('.$sysID.',this,'.$getEventArray.')" id='.$sysID.'_'.$parentID.' data='.$parentID.' class="evenetClass" name="events[]"  value="'.$sysID.'" style="float: left;margin: 5px -17px;"/>';
