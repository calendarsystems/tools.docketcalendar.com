...
if (!function_exists("GetSQLValueString")) {
	...
	
	{
	  if (PHP_VERSION < 6) {
	   ...
	   $docketData = $GLOBALS['docketData'];$theValue = function_exists("mysqli_real_escape_string") ? mysqli_real_escape_string($datavalue,$theValue) : mysqli_escape_string($datavalue,$theValue);
...
$jurisdicationVal = $_POST['jurisdicationVal'] ;
...
if (isset($_SESSION['userid']))
{
	...
	$command="/triggers?";
	$parameters="loginToken=$loginToken&jurisdictionSystemID=$jurisdicationVal";
	$file= $newURL.$command.$parameters;
	...
	$theTriggers=$array['ArrayOfTriggerItem']['TriggerItem'];
			...
			if($jurisdicationVal)
			{
				...
				$result.="<tr><td class='reviewheader'>Trigger Name</td><td class='reviewheader'>Jurisdiction</td><td class='reviewheader'>Action</td></tr>";
				$query_searchInfo = "SELECT distinct(trigger_item) as TriggerID,juriapply FROM exclude_events WHERE jurisdiction = '".$jurisdicationVal."' and createdBy = '".$userName."'";
				...
				$totalRows_searchInfo = mysqli_num_rows($searchInfo);
							if($totalRows_searchInfo > 0)
							{
								...
								while($rowjusriInfo = mysqli_fetch_assoc($searchInfo))	
									{			
										foreach($theTriggers as $trigerVal)
										{
											if($rowjusriInfo['TriggerID'] == $trigerVal['SystemID'])
											{
												if($rowjusriInfo['juriapply'] == 1)
												{
													$juriType = "Single";
												}
												else{
												...
												$result.="<tr><td>".$trigerVal['Description']."</td><td>".$juriType."</td><td><a href='viewexlevent?trig=".$rowjusriInfo['TriggerID'].'&juri='.$jurisdicationVal."'>VIEW</a></td></tr>";
											}
