...
if($_POST['case_matter'] != "")
{
	...
	{		
		...
		if (!empty($_POST['attendee'])) {
				...
				foreach($attendee as $attendeeVal)
					...
					$queryCaseInfo = "SELECT * FROM docket_cases WHERE case_id = '".$caseId."' ";
					...
					if($totalRows_caseInfo > 0)
					{
						...
						$reminderEmail = $rowCaseInfo['caseReminderTime'];
						$reminderPopUp = $rowCaseInfo['reminder_minutes_popup'];
					}else{
						$queryUserToolInfo="SELECT * from users_tool_option WHERE user_id = ".$rowCaseInfo['user_id']."";
						...
						$reminderEmail = $rowEventInfo['reminder_minutes'];
						$reminderPopUp = $rowEventInfo['reminder_minutes_popup'];
					}
					

		$callToUpdateCasedetails = UpdateCaseDetails($caseId,$capi,$attendee,$eventColor,$reminderEmail,$reminderPopUp);
	...
	function UpdateCaseDetails($caseId,$capi,$attendee=null,$eventColor=null,$reminderEmail=null,$reminderPopUp=null)
	{
		...
		if($countAllImportDocketIdForCase > 0)
		{
			...
			{
				$getAllEventsIdForImportDocketId="SELECT event_id,import_event_id,system_id  from  import_events WHERE import_docket_id = ".$rowAllImportDocketIdForCase['import_docket_id']."";
				...
				while($rowEventsIdForImportDocketIdData = mysqli_fetch_assoc($AllEventsIdForImportDocketIdData))
				{
					...
					$querygetupdateeventsdescInfo = "SELECT * FROM updateeventsdesc WHERE importdocketid = '".$rowAllImportDocketIdForCase['import_docket_id']."' AND caseid = ".$caseId." AND  eventid IN (0,'".$rowEventsIdForImportDocketIdData['system_id']."')";
					...
					if($totalRowsupdateeventsdescInfo > 0)
					{
						...
						if($row_getupdateeventsdesc['eventColor'] > 0)
						{
							 $eventColorChg = $row_getupdateeventsdesc['eventColor'];
						}
