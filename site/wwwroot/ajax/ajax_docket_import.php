...
if (!function_exists("GetSQLValueString")) 
{
	...
	if($serialize_events != '')
	{
		...
		$result = mysqli_query($docketDataSubscribe,$updateSQL) or trigger_error("Query Failed! SQL: $insertSQL - Error: ".mysqli_error(mysqli_error), E_USER_ERROR);
	}
	...
	/*
	if(!empty($_POST['caseLevelImporteventColor']))
	{
		
		$updateEventColorSQL = "UPDATE docket_cases SET caseEventColor='".mysqli_real_escape_string($docketDataSubscribe,$_POST['caseLevelImporteventColor'])."' WHERE case_id = ".$CaseID."";
	...
	*/
	...
	$query_userInfo = sprintf("SELECT * FROM attorneys WHERE user_id = %s", GetSQLValueString($docketDataSubscribe,$colname_userInfo, "int"));
	...
	$sqlSelectCaseCustomText = "SELECT case_customtext FROM docket_customtext WHERE case_id =".$row_searchInfo['case_id']." AND case_customtextlevel=1";
					...
					if($totalRowsSelectCaseCustomText > 0)
					{
						...
						{
							 $textValue = "Case Custom Comments : ".$rowData ['case_customtext']."<br/>";
						}	
					}
	...
	//    echo "juris good";
		...
		}else
		{
			if(!empty($_POST['caseLevelImporteventColor']))
			{
				$eventColor = $_POST['caseLevelImporteventColor'];
			}else{
		...
		$LinkToEditCaseEvent = "<br>https://tools.docketcalendar.com/view-calendar-event?id=";
		$maxCaseEventId = $maxCaseEventId + 1;
		$EditCaseEventLink = $LinkToEditCaseEvent.$maxCaseEventId.'<br>';
							
		$description = $textValue.$TriggerData.' '.$EditCaseEventLink.' '.$case_name_append_before.' '.$custom_text_append_before.' '.$court_rule. ' ' .$date_rule.' '.$triggerItemCalc.' '.$case_name_append_after.' '.$custom_text_append_after
		...
		//echo $addReminderFlag ;exit();
		/* VIEW LINK SEND EMAIL TO ATTENDEES AND ALSO CREATE EVENT IN CALENDAR */
		$event_id = $capi->CreateCalendarEvent(''.$_POST['calendar_id'].'', $summary, $all_day, $date_array, $user_timezone, $_SESSION['access_token'], $attendees, $description,$reminder_minutes,$reminder_popup,$eventColor,$location,$status,$addReminderFlag,$addReminderVal,$addPopUpReminderVal);
	...
	if($events_array != '')
	{
		...
		{ 
			  ...
			  $queryForMultipleUpdateEvents="SELECT * FROM updateeventsdesc WHERE importdocketid=".$_POST['docket_search_id']." AND caseid = ".$row_searchInfo['case_id']." AND eventid = '".$sysID."'";
			...
			$eventTypeDesc = $Event['EventType'];
				...
				$event_id = $capi->CreateCalendarEvent(''.$_POST['calendar_id'].'', $summary, $all_day, $date_array, $user_timezone, $_SESSION['access_token'],$attendees,$textValue.$description,$reminder_minutes,$reminder_popupminutes,$eventColor,$location,$status,$addReminderFlag,$addReminderVal,$addPopUpReminderVal);
