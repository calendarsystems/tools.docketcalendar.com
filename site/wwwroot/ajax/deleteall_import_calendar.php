...
foreach($case_event_id as $Casevalue) {
    $query_importEvents = "SELECT e.event_id,i.calendar_id,e.import_docket_id
    ...
    WHERE c.case_event_id = ".$Casevalue." ";
    ...
    if($totalRows_searchInfo > 0 && $row_searchInfo['event_id'] != '')
    {

             $data = $capi->DeleteCalendarEvent($row_searchInfo['event_id'],$row_searchInfo['calendar_id'],$_SESSION['access_token']);
        
             $query_deleteInfo = "DELETE FROM case_events WHERE case_event_id = ".$Casevalue." ";
        ...
        if($row_searchInfo['import_docket_id'])
        {
            
            $query_importDocket = "SELECT import_event_id FROM import_events WHERE import_docket_id = '".$row_searchInfo['import_docket_id']."' ";
            ...
            {
               $query_deleteDocket = "DELETE FROM import_docket_calculator WHERE import_docket_id = '".$row_searchInfo['import_docket_id']."' ";
