...
if (isset($_SESSION['userid']))
{
	...
	if($caseId)
	{
			$query_authInfo = "SELECT case_jurisdiction FROM docket_cases where case_id = '".$caseId."'";
			...
			if($totalRows_authInfo > 0)
			{
			  ...
			  $default_court_system_id = $row_authInfo['case_jurisdiction'];
			}
	}else{
		...
		if (isset($_POST['cmbJurisdictions']))
		  {
			  $default_court_system_id = $_POST['cmbJurisdictions'];
		  }
	}
	
    ?>	
		<select name="case_jurisdiction" id="case_jurisdiction"  style="width:380px;" >
       ...
       <?php }
        ...
        ?>
               ...
               { echo 'selected = "selected"'; }
                 if ( (isset($_POST['cmbJurisdictions']) && $masterCourt==$juris['SystemID']) || ($default_court_system_id == $juris['SystemID']))
                 { echo 'selected = "selected"'; } ?>>
  ...
  ?>
	 ...
	 <input type="hidden" id="selectJusrivalue"	name="selectJusrivalue"/>	
	<script>
		jQuery('#case_jurisdiction').on('change', function() {
		var selectValue = jQuery('#case_jurisdiction option:selected').val();
			jQuery("#selectJusrivalue").val(selectValue);
		})
	</script>
