		...
		if($caseId)
		{
			...
			if(!empty($_POST['attendees']))
			{
				...
				foreach($attendee as $attendeeVal)
				{
					...
					GetSQLValueString($docketDataSubscribe,$caseId, "text"),
					GetSQLValueString($docketDataSubscribe,$attendeeVal, "text"),
					GetSQLValueString($docketDataSubscribe,$jurisid, "int"),
					GetSQLValueString($docketDataSubscribe,$triggerid, "int"),
				...
				$eventColor =$row_caseInfo['caseEventColor'];
					...
					foreach($TotalJurisdictionsList as $jurisData)
					{
						...
						/* VIEW LINK SEND EMAIL TO ATTENDEES AND ALSO CREATE EVENT IN CALENDAR */
							...
							$trigger_event_id = $capi->CreateCalendarEvent(''.$calendar_id.'', $summary, $all_day, $date_array, $user_timezone, $_SESSION['access_token'],$attendees,"Trigger Custom Comments : ".mysqli_real_escape_string($docketDataSubscribe,$_POST['triggerCustomText'])."<br/>".$description,$reminder_minutes,$reminder_popup,$eventColor,$location,$status,$addReminderFlag,$addReminderVal,$addPopUpReminderVal);
			...
			if($caseLeveleventColor == NULL || $caseLeveleventColor == 0)
			{
				...
				$eventColor =$row_caseInfo['caseEventColor'];
						...
						$trigger_event_id = $capi->CreateCalendarEvent(''.$calendar_id.'', $summary, $all_day, $date_array, $user_timezone, $_SESSION['access_token'],$attendees,"Trigger Custom Comments : ".mysqli_real_escape_string($docketDataSubscribe,$_POST['triggerCustomText'])."<br/>".$description,$reminder_minutes,$reminder_popupminutes,$eventColor,$location,$status,$addReminderFlag,$addReminderVal,$addPopUpReminderVal);
					...
					$responseHtml = "<span style='color:green;'>Successfully Update Trigger.</span>";
