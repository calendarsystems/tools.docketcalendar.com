...
try
{
	...
	//echo '<BR>profileID: '. $profilchrgAmounte_id. '<BR>paymentID:'. $payment_id.'<BR>ShippingID:'.$shipping_profile_id;
...
//  this is where you set the receipt info /////////////////////////////////////////////////////////////////
	...
	$cim->setParameter('orderDescription', 'Convert Trial Account');
			...
			do {
				...
				// echo '<BR>NEW: '. $row_attornys_cart['name'] .' - '.$row_attornys_cart['username'].' - '.$row_attornys_cart['password'].' - '.$_POST['firm'].'<BR>';
					...
					include('../include/inc_add_subuser_to_crc.php'); 
					 ...
					 // update users cart to mark subscribed courts
					  $updateSQL = sprintf("UPDATE cart SET subscribed=%s, TrialAccount=0 WHERE sessionid=%s",
				...
				if (empty($row_attornys_cart['username'])) {
					...
					$pass = $row_attornys_cart['password'];
						...
						$userToken = new Pest($CRCurl);
						$userToken = $userToken->get('/users/'.$row_attornys_cart['username'].'?password='.$row_attornys_cart['password'].'&soapREST=SOAP');
						$userToken = new SimpleXMLElement($userToken);
